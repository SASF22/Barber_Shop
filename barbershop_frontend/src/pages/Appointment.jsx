            ...
            const generateMonth = ()=>{                  
                ...
                return(
                    ...
                    {monthObject.days.map((item, num)=>{
                        return (
                        <div className="daySquare" dataDate={{date:item.date,day:item.day,year:item.year}} onClick={(e)=>{return handleDayClick(e)}}    style={{display: 'flex', justifyContent:'center', border: '2px solid black', borderRadius:'.5rem', width:"calc(14.28% - 4px)"}}>{item.day} {item.date}</div>
                    )
                    })}       
                    </>
                )
